 ...
 */
public class
        OpenstackCollectorUsageDataRecordResource extends ServerResource {
    final static Logger logger = LogManager.getLogger(OpenstackCollectorUsageDataRecordResource.class.getName());
    private String endpoint = "/openstackcoll/usage";
    private APICallCounter counter = APICallCounter.getInstance();

    public OpenstackCollectorUsageDataRecordResource() {
        super();
    ...
    public void generateUDR(String clientId, String instanceId) {
        ...
        OpenstackCollectorResource OpenstackCollectorResource = new OpenstackCollectorResource();
        DateTimeUtil dateTimeUtil = new DateTimeUtil();
        String[] range = dateTimeUtil.getRange();
        long computeUDRFrom = dateTimeUtil.getEpoch(range[0]);
        ArrayList<Event> value = fromTSDBtoEvents(OpenstackCollectorResource.captureLastEventBeforeDate(clientId, instanceId, computeUDRFrom));
        Event lastEvent = value.get(0);
    ...
    public void computeUDR(Event event) {// (Representation entity) {
        try {
            ...
            if (lastEventDate < computeUDRFrom) {
                ...
                tsdbResource.saveMcnUDR(event, usedSeconds, computeUDRFrom, computeUDRTo, false);
            } else {
                //Check if i only have 1 event in the time period
                ArrayList<Event> events = fromTSDBtoEvents(OpenstackCollectorResource.captureEventsAfterDate(clientId, instanceId, new Date(computeUDRFrom)));
                if (events.size() > 1) {
    ...
    private boolean usageEvent(Event event) {
        return (event.getStatus().equals(Loader.getSettings().getOpenstackCollectorSettings().getOpenstackCollectorEventStart()) ||
                event.getStatus().equals(Loader.getSettings().getOpenstackCollectorSettings().getOpenstackCollectorEventSpawn()) ||
                event.getStatus().equals(Loader.getSettings().getOpenstackCollectorSettings().getOpenstackCollectorEventUnpause()) ||
                event.getStatus().equals(Loader.getSettings().getOpenstackCollectorSettings().getOpenstackCollectorEventResume()));
    }
    ...
    private void computeAndSaveOneEvent(ArrayList<Event> events, DateTimeUtil dateTimeUtil, TSDBResource tsdbResource, long computeUDRFrom, long computeUDRTo, Event event) {
        ...
        if (usageEvent(event)) {//TODO: format all the events in the same way or have a constant MCN.START TNOVA.START
        ...
        tsdbResource.saveMcnUDR(events.get(0), usedSeconds, computeUDRFrom, computeUDRTo, flagSetupCost);
    }
    ...
    private void computeAndSaveUsageMultipleEvents(ArrayList<Event> events, DateTimeUtil dateTimeUtil, TSDBResource tsdbResource, long computeUDRFrom, long computeUDRTo) {
        ...
        for (int i = 0; i < events.size(); i++) {
            ...
            //Generate UDR
            tsdbResource.saveMcnUDR(events.get(i), usedSeconds, computeUDRFrom, computeUDRTo, flagSetupCost);
        }
    }
