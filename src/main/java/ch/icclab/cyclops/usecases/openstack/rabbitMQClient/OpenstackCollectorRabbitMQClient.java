 ...
 * @date 18.08.2015
 */
package ch.icclab.cyclops.usecases.openstack.rabbitMQClient;
...
import ch.icclab.cyclops.usecases.openstack.model.OpenstackCollectorEvent;
 ...
 */
public class OpenstackCollectorRabbitMQClient implements Runnable {
    final static Logger logger = LogManager.getLogger(OpenstackCollectorRabbitMQClient.class.getName());
     ...
     */
    public OpenstackCollectorRabbitMQClient(String queueName) {
     ...
     */
    private OpenstackCollectorEvent processMessage(String message) {
        Gson gson = new Gson();

        OpenstackCollectorEvent data = gson.fromJson(message, OpenstackCollectorEvent.class);
        ...
        return new DefaultConsumer(channel) {
            ...
            public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                try {
                    ...
                    // now parse it
                    OpenstackCollectorEvent openstackCollectorEvent = processMessage(message);

                    if (openstackCollectorEvent != null) {
                        // request empty container
                        BatchPoints container = db.giveMeEmptyContainerForEvents(Loader.getSettings().getOpenstackCollectorSettings().getOpenstackCollectorDBEventsName());
                        // add a point
                        container.point(openstackCollectorEvent.toPoint());
