 ...
 */
public class McnThreadExecutor {

    final static Logger logger = LogManager.getLogger(McnThreadExecutor.class.getName());

    // this class has to be a singleton
    private static McnThreadExecutor singleton = new McnThreadExecutor();

    // executor service (we only need one thread)
    private ExecutorService executor;

    // RabbitMQ object that will be used for listening
    private McnRabbitMQClient rabbitmq;

    /**
     * We need to hide constructor from public
     */
    private McnThreadExecutor() {
        this.executor = null;
    }
     ...
     */
    public static McnThreadExecutor getInstance() {
        return singleton;
    }
    ...
    public void start(String queueName) {
        if (executor == null) {
            ...
            this.rabbitmq = new McnRabbitMQClient(queueName);
